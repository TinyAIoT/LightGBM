...
def fetch_data_small(data, target, name, seed, flatten=False):
...
def fetch_data(data, target, name, random, flatten=False):
    ...
    X_train, X_test, y_train, y_test = train_test_split(X_trainpre, y_trainpre, test_size=0.1, random_state=random)
...
if __name__ == "__main__":
    ...
    for seed in seeds:
        ...
        if not os.path.exists(directory + '/covtype.test'):
            ...
            fetch_data(covtype_libsvm[0], covtype_libsvm[1], 'covtype', seed)
        ...
        if not os.path.exists(directory + '/california_housing.test'):
            ...
            fetch_data(housing.data, housing.target, 'california_housing', seed)
        ...
        if not os.path.exists(directory + 'kin8nm.test'):
            ...
            fetch_data(kin8nm.data, kin8nm.target, 'kin8nm', seed)
        ...
        if not os.path.exists(directory + '/mushroom.test'):
            ...
            fetch_data(X, y, 'mushroom', seed, True)
        ...
        if not os.path.exists(directory + '/kr-vs-kp.test'):
            ...
            X = X.apply(LabelEncoder().fit_transform).to_numpy()
            y = y.apply(LabelEncoder().fit_transform).to_numpy()
            fetch_data_small(X, y, 'kr-vs-kp', seed, True)
        ...
        if not os.path.exists(directory + '/breastcancer.test'):
            ...
            X = X.apply(LabelEncoder().fit_transform).to_numpy()
            y = y.apply(LabelEncoder().fit_transform).to_numpy()
            fetch_data_small(X, y, 'breastcancer', seed,True)
        ...
        if not os.path.exists(directory + '/covtype_multi.test'):
            ...
            X = X.to_numpy()
            y = y.apply(LabelEncoder().fit_transform).to_numpy()
            fetch_data(X, y, 'covtype_multi', seed, True)
